...
class BookingEventEdit extends Component {
  ...
  handleSubmit() {
    ...
    // 进行其他处理或者发起登录请求
      $.ajax({
        url: 'http://localhost:8080/music_war_exploded/customerCancelBookEvent',
        method: 'POST',
        data: {
          user_id: user_id,
          event_id: event_id,
    ...
    return (
      <div>
        <Form name="normal_login" className="event-form">
                ...
                <Form.Item>
                  <Button type="primary"  className="c-booking-form-submit-button" onClick={this.handleSubmit.bind(this)}>
                    Cancel
                  </Button>
                </Form.Item>
              </Form>
