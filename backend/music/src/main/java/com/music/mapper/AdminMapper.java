...
public class AdminMapper {
    ...
    public List<User> adminFindAllUsers() {
        //sql inject
        String sql = "SELECT * FROM public.user where public.user.delete=0";
    ...
    public List<Event> adminFindAllEvents() {
        //sql inject
        String sql = "SELECT * FROM public.event WHERE public.event.delete=0";
        try(Connection connection = jdbcUtils.getConnection();
            PreparedStatement statement = connection.prepareStatement(sql)){
    ...
    public Boolean isRepeatVenueName(Venue venue) {
        //sql inject
        String sql = "SELECT * FROM public.venue where venue_name='"+venue.getVenueName()+"' and public.venue.delete=0";
        try(Connection connection = jdbcUtils.getConnection();
            PreparedStatement statement = connection.prepareStatement(sql)){
    ...
    public List<Venue> findAllVenue() {
        //sql inject
        String sql = "SELECT * FROM public.venue WHERE public.venue.delete=0";
        try(Connection connection = jdbcUtils.getConnection();
            PreparedStatement statement = connection.prepareStatement(sql)){
    ...
    public void addAllVenueToAllVenueSections(List<Venue> venueList) {
        for (int i = 0; i < venueList.size(); i++) {
            //sql inject
            String sql = "SELECT * FROM public.venue_section WHERE public.venue_section.delete=0 and venue_id="+venueList.get(i).getId();
            try(Connection connection = jdbcUtils.getConnection();
                PreparedStatement statement = connection.prepareStatement(sql)){
    ...
    public List<UserBookingEvent> findAllUserBookingEvent() {
        String sql="select * from user_booking_event where user_booking_event.delete=0 and user_booking_event.booking=1";
        try(Connection connection = jdbcUtils.getConnection();
            PreparedStatement statement = connection.prepareStatement(sql)){
    ...
    public User findUserByUserId(UserBookingEvent userBookingEvent) {
        //sql inject
        String sql = "SELECT * FROM public.user where id="+userBookingEvent.getUserId();
        try(Connection connection = jdbcUtils.getConnection();
            PreparedStatement statement = connection.prepareStatement(sql)){
