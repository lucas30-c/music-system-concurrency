    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if ("/eventPlannerAddEvent".equals(request.getServletPath())) {
            try {
                ...
                Date parsedDate = dateFormat.parse(start_time); // 解析日期时间字符串
                event.setStartTime(new Timestamp(parsedDate.getTime()));
                parsedDate = dateFormat.parse(end_time); // 解析日期时间字符串
...
//                System.out.println(jsonObject.toJSONString());
                    ...
                    if(isConflict) {//如果该场地有冲突  那么就返回创建失败的信息
                        response.setStatus(HttpServletResponse.SC_CONFLICT);
...
//                            eventPlannerService.updateVenueSectionPriceByVenueIdAndId(section);
                        }
                        // ready to return
                        List<Event> events = eventPlannerService.findAllEvents();
                        for(int i = 0; i < events.size(); i++) {
            ...
            try{
                ...
                SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                Date parsedDate = dateFormat.parse(startTime); // 解析日期时间字符串
                event.setStartTime(new Timestamp(parsedDate.getTime()));
                parsedDate = dateFormat.parse(endTime); // 解析日期时间字符串
                event.setEndTime(new Timestamp(parsedDate.getTime()));
                eventPlannerService.updateEvent(event);//更新活动属性
